...
if($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    {
        $lastnameerr = "Please enter your Last Name can't be empty!";
    }
    ...
    else if(empty($_POST['nnid']))
    {
        ...
        for($i = 0; $i< count($data_filter)-1; $i++) {
            ...
            if($json_decode['email'] == $username) 
            {
                $usertable = array('firstname' => $firstname, 'lastname' => $lastname, 'nid' => $nid, 'dob' => $dob, 'gender' => $gender, 'email' => $email, 'phone' => $phone, 'nnid' => $nnid, 'type' => $type);
